fwEvents.one('fw-builder:' + 'quiz-builder' + ':register-items', function (builder) {
	var localized = fw_quiz_builder_item_type_short_answer;

	var ItemView = builder.classes.ItemView.extend({
		template: _.template(
			'<div class="fw-quiz-builder-item-style-default fw-quiz-builder-item-type-short-answer">' +
			...
			'<div class="fw-quiz-item-preview-title-wrapper"><label><%- answer %></label></div>' +
			'</div>' +
			'</div>' +
			'</div>'
		),
		events: {
			...
			'click .fw-quiz-builder-item-type-short-answer': 'openEdit',
		...
		acceptedAnswers: function () {
			var answers = fw.opg('accepted-answers', this.model.get('options'));

			if (!_.isArray(answers)) {
				return [];
			}

			return _.filter(answers, function (answer) {
				return (answer || '').toString().trim().length > 0;
			});
		},
		parseAnswers: function () {
			var answers = this.acceptedAnswers();

			if (answers.length === 0) {
				return localized.l10n.name;
			}

			return fwQuizBuilder.esc_attr(answers.join(', '));
		}
	});
